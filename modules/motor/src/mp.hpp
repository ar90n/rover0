...
template<unsigned N, unsigned K, typename T, typename Tag>
struct setter
{
  constexpr friend auto element_func(reader<N, Tag>) { return std::make_tuple(K, T{}); }

  static constexpr element_t<N, K, T> element{};
};

template<unsigned N, typename Tag, auto EvalTag>
...
{
  constexpr bool counted_past_n = requires(reader<N, Tag> r) { element_func(r); };
...
template<unsigned K, typename Tag, auto EvalTag = [] {}, auto State = find_element_impl<K, Tag, EvalTag>()>
