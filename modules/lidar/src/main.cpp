...
using UartControl = Uart<Config::UART_LIDAR, Config::UART_CONTROL_TX_PIN,
                         Config::UART_CONTROL_RX_PIN, Config::UART_BUFFER_SIZE>;
using UartLidar = Uart<Config::UART_CONTROL, Config::UART_LIDAR_RX_PIN,
                       Config::UART_LIDAR_RX_PIN, Config::UART_BUFFER_SIZE>;
...
xv11::Lidar lidar(read_byte_from_serial, write_pwm_value, time_us_32,
                  Config::LIDAR_RPM);
...
void fetch_timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
  ...
  while (lidar.process(&packet)) {
    ...
    for (int i = 0; i < 4; i++) {
      distances[4 * lidarAngle + i] =
          (packet.distances[i] / 1000.0); // Distance to object in meters
    }
...
int main() {
  ...
  rmw_uros_set_custom_transport(true, &trns, uRosTransport::open,
                                uRosTransport::close, uRosTransport::write,
                                uRosTransport::read);
  // Wait for agent successful ping for 2 minutes.
  auto ret =
      rmw_uros_ping_agent(Config::UROS_TIMEOUT_MS, Config::UROS_ATTEMPTS);
  ...
  if (0 < Config::ROS_DOMAIN_ID) {
    RCCHECK(rcl_init_options_set_domain_id(
        &init_options, static_cast<size_t>(Config::ROS_DOMAIN_ID)));
  }
  ...
  const unsigned int publish_timer_timeout =
      60.0 / Config::LIDAR_RPM * 1000 + 10;
  ...
  const unsigned int fetch_timer_timeout =
      (60.0 / Config::LIDAR_RPM * 1000) / 90 * 5;
  RCCHECK(rclc_timer_init_default(&fetch_timer, &support,
