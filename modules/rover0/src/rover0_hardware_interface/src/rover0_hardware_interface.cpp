...
std::array<uint32_t, 6> const imu_queries{ message::serialize(message::ImuMsg{
...
rover0_hardware_interface::WheelState::WheelState(std::string& name, int16_t encoder_tics_per_revolution)
  ...
  , velocity{ 0.0 }
  , name{ name }
  , encoder_tics_per_revolution{ encoder_tics_per_revolution }
{
}

void rover0_hardware_interface::WheelState::insert_state_interfaces(
...
{
  double const delta_position =
    2.0 * std::numbers::pi * static_cast<double>(delta_tics) / encoder_tics_per_revolution;
...
void rover0_hardware_interface::WheelCommand::insert_command_interfaces(
...
rover0_hardware_interface::WheelCommand::create_velocity_command_interface()
{
...
void rover0_hardware_interface::IMUState::insert_state_interfaces(
  std::vector<hardware_interface::StateInterface>& state_interfaces
)
{
  ...
  switch (param)
  {
    ...
    case message::ImuData::ACCEL_Z:
      linear_acceleration_[2] =
        calc_linear_acceleration(value, 0.0); // skip offset for z-axis because of gravity
      break;
...
rover0_hardware_interface::IMUState::create_angular_velocity_x_state_interface()
{
  return hardware_interface::StateInterface(name, "angular_velocity.x", &angular_velocity_[0]);
}

hardware_interface::StateInterface
rover0_hardware_interface::IMUState::create_angular_velocity_y_state_interface()
{
  return hardware_interface::StateInterface(name, "angular_velocity.y", &angular_velocity_[1]);
}

hardware_interface::StateInterface
rover0_hardware_interface::IMUState::create_angular_velocity_z_state_interface()
{
  return hardware_interface::StateInterface(name, "angular_velocity.z", &angular_velocity_[2]);
}

hardware_interface::StateInterface
rover0_hardware_interface::IMUState::create_linear_acceleration_x_state_interface()
{
  return hardware_interface::StateInterface(name, "linear_acceleration.x", &linear_acceleration_[0]);
}

hardware_interface::StateInterface
rover0_hardware_interface::IMUState::create_linear_acceleration_y_state_interface()
{
  return hardware_interface::StateInterface(name, "linear_acceleration.y", &linear_acceleration_[1]);
}

hardware_interface::StateInterface
rover0_hardware_interface::IMUState::create_linear_acceleration_z_state_interface()
{
  return hardware_interface::StateInterface(name, "linear_acceleration.z", &linear_acceleration_[2]);
}

hardware_interface::CallbackReturn rover0_hardware_interface::Rover0HardwareInterface::on_init(
...
)
{
  ...
  wheel_states_.emplace(
    message::MotorDevice::REAR_LEFT,
    WheelState(config_.rear_left_wheel_joint_name, config_.encoder_tics_per_revolution)
  );
  wheel_states_.emplace(
    message::MotorDevice::REAR_RIGHT,
    WheelState(config_.rear_right_wheel_joint_name, config_.encoder_tics_per_revolution)
  );
  wheel_states_.emplace(
    message::MotorDevice::FRONT_LEFT,
    WheelState(config_.front_left_wheel_joint_name, config_.encoder_tics_per_revolution)
  );
  wheel_states_.emplace(
    message::MotorDevice::FRONT_RIGHT,
    WheelState(config_.front_right_wheel_joint_name, config_.encoder_tics_per_revolution)
  );

  wheel_commands_.emplace(message::MotorDevice::REAR_LEFT, WheelCommand(config_.rear_left_wheel_joint_name));
  wheel_commands_.emplace(
    message::MotorDevice::REAR_RIGHT,
    WheelCommand(config_.rear_right_wheel_joint_name)
  );
  wheel_commands_.emplace(
    message::MotorDevice::FRONT_LEFT,
    WheelCommand(config_.front_left_wheel_joint_name)
  );
  wheel_commands_.emplace(
    message::MotorDevice::FRONT_RIGHT,
    WheelCommand(config_.front_right_wheel_joint_name)
  );
  ...
  for (const hardware_interface::ComponentInfo& joint : info.joints)
  {
    ...
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("rover0_hardware_interface"),
    ...
    if (joint.command_interfaces[0].name != hardware_interface::HW_IF_VELOCITY)
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("rover0_hardware_interface"),
        "Joint '%s' has command interface '%s', 'velocity' expected",
        joint.name.c_str(),
    ...
    if (joint.state_interfaces[0].name != hardware_interface::HW_IF_POSITION)
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("rover0_hardware_interface"),
        "Joint '%s' has state interface '%s', 'position' expected",
        joint.name.c_str(),
...
rover0_hardware_interface::Rover0HardwareInterface::on_configure(const rclcpp_lifecycle::State&)
{
  ...
  struct termios tio
  {
    .c_iflag = 0, .c_oflag = 0, .c_cflag = CREAD | CLOCAL | CS8, .c_lflag = 0, .c_line = 0, .c_cc = { 0 },
  ...
  transport::init([this](const uint8_t* data, size_t size) { ::write(this->fd_, data, size); });

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn
rover0_hardware_interface::Rover0HardwareInterface::on_cleanup(const rclcpp_lifecycle::State&)
{
...
hardware_interface::CallbackReturn
rover0_hardware_interface::Rover0HardwareInterface::on_activate(const rclcpp_lifecycle::State&)
{
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn
rover0_hardware_interface::Rover0HardwareInterface::on_deactivate(const rclcpp_lifecycle::State&)
{
  return hardware_interface::CallbackReturn::SUCCESS;
}

std::vector<hardware_interface::StateInterface>
rover0_hardware_interface::Rover0HardwareInterface::export_state_interfaces()
{
  std::vector<hardware_interface::StateInterface> state_interfaces;
...
rover0_hardware_interface::Rover0HardwareInterface::export_command_interfaces()
{
  std::vector<hardware_interface::CommandInterface> command_interfaces;
...
rover0_hardware_interface::Rover0HardwareInterface::read(const rclcpp::Time&, const rclcpp::Duration& period)
{
  read_call_count_++;
  transport::reset();
  ...
  while (true)
  {
    ...
    for (int i = 0; i < len; i++)
    {
      ...
      {
        handle_encoder_message(
          message::deserialize<message::EncoderMsg>(recv.value()),
          motor_query_delta_secs
        );
      }
    }
  }
...
hardware_interface::return_type
rover0_hardware_interface::Rover0HardwareInterface::write(const rclcpp::Time&, const rclcpp::Duration&)
{
  for (auto& command : wheel_commands_)
  {
    double const rotate_per_sec =
      command.second.velocity / (2.0 * std::numbers::pi); // command velocity is in rad/s
    ...
    transport::send(message::serialize(message::MotorMsg{ .param = command.first, .value = tics_per_sec_q7 })
    );
  }
  return hardware_interface::return_type::OK;
}
...
void rover0_hardware_interface::Rover0HardwareInterface::handle_encoder_message(
  const message::EncoderMsg& encoder_msg,
  double                     delta_secs
)
{
...
PLUGINLIB_EXPORT_CLASS(
  rover0_hardware_interface::Rover0HardwareInterface,
