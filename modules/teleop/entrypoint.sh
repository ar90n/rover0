...
if [ "$ENV" = "build" ]; then
  ...
  rosdep update
  rosdep install -y -r --from-paths src --skip-keys libcamera -t build -t buildtool
  colcon build
  exit 0
elif [ "$ENV" = "dev" ]; then
  echo "Running colcon build --symlink-install..."
  rosdep update
  rosdep install -y -r --from-paths src --skip-keys libcamera -t build -t buildtool -t exec
