...
public:
    ...
    requires mp::SameValue<FUNC, GPIO_FUNC_PWM>
  {
    ...
    uint16_t level = static_cast<uint16_t>(
        std::round(pwm_hw->slice[slice_num].top * clamped_value));
    ...
    requires mp::SameValue<FUNC, GPIO_FUNC_PWM>
  {
    ...
    auto const a_inverted{
        (pwm_hw->slice[slice_num].csr & PWM_CH0_CSR_A_INV_BITS) != 0};
    auto const b_inverted{
        (pwm_hw->slice[slice_num].csr & PWM_CH0_CSR_B_INV_BITS) != 0};
