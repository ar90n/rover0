...
template <MsgType T, typename TP, typename TV> struct Message {
  using param_type = TP;
  using value_type = TV;
...
using RxMsg = std::variant<MotorMsg>;
using TxMsg = std::variant<EncoderMsg, ImuMsg>;

template <typename T> uint32_t serialize(T const &msg) {
  union {
    uint32_t raw;
    struct {
      MsgType type;
...
template <typename T> T deserialize(uint32_t raw) {
  union {
    uint32_t raw;
    struct {
      MsgType type;
